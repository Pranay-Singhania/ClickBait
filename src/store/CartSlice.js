...
const CartSlice = createSlice({
  ...
  initialState: {
    ...
    totalItems: 0,
    totalAmount: 0,
    totalDeliveryCharge: 80,
  },
  reducers: {
    addToCart: (state, action) => {
      const tempItem = state.data.find((item) => item.id === action.payload.id); //check whether the added item is already in the cart or not by prod id
      if (tempItem) {
